...
<script runat="server">
    ...
    void GetGrid()
    {
        ...
        con.Open();
        // создание команды
        SqlCommand cmd = new SqlCommand("SELECT KDRNKZ.* FROM KDR INNER JOIN KDRNKZ ON KDR.KDRKOD=KDRNKZ.KDRKOD WHERE KDR.KDRKOD=" + KdrKod, con);
    ...
    {
        NkzBeg = Convert.ToString(e.Record["NKZBEG"]);
        NkzMem = Convert.ToString(e.Record["NKZMEM"]);
        NkzEnd = Convert.ToString(e.Record["NKZEND"]);
        ...
        // создание команды
        SqlCommand cmd = new SqlCommand("INSERT INTO KDRNKZ (KDRKOD,NKZEND,NKZBEG,NKZMEM) " +
                                        "VALUES (@KDRKOD,CONVERT(DATETIME,@NKZEND,103),CONVERT(DATETIME,@NKZBEG,103),@NKZMEM)", con);
        ...
        cmd.Parameters.Add("@NKZBEG", SqlDbType.VarChar).Value = NkzBeg;
        cmd.Parameters.Add("@NKZEND", SqlDbType.VarChar).Value = NkzEnd;
        cmd.Parameters.Add("@NKZMEM", SqlDbType.VarChar).Value = NkzMem;
    ...
    {
        NkzIdn = Convert.ToInt32(e.Record["NKZIDN"]);
        NkzBeg = Convert.ToString(e.Record["NKZBEG"]);
        NkzMem = Convert.ToString(e.Record["NKZMEM"]);
        NkzEnd = Convert.ToString(e.Record["NKZEND"]);
        ...
        // создание команды
        SqlCommand cmd = new SqlCommand("UPDATE KDRNKZ SET NKZBEG=CONVERT(DATETIME,@NKZBEG,103),NKZEND=CONVERT(DATETIME,@NKZEND,103),NKZMEM=@NKZMEM WHERE NKZIDN=@NKZIDN", con);
        //   cmd = new SqlCommand("BuxSprKdrAdd", con);
        // указать тип команды
        //      cmd.CommandType = CommandType.StoredProcSemre;
        // создать коллекцию параметров

        cmd.Parameters.Add("@NKZIDN", SqlDbType.VarChar).Value = NkzIdn;
        cmd.Parameters.Add("@NKZBEG", SqlDbType.VarChar).Value = NkzBeg;
        cmd.Parameters.Add("@NKZMEM", SqlDbType.VarChar).Value = NkzMem;
        cmd.Parameters.Add("@NKZEND", SqlDbType.VarChar).Value = NkzEnd;
        // ------------------------------------------------------------------------------заполняем второй уровень
    ...
    void DeleteRecord(object sender, GridRecordEventArgs e)
    {
        ...
        // создание команды
        SqlCommand cmd = new SqlCommand("DELETE FROM KDRNKZ WHERE NKZIDN=@NKZIDN", con);
        //   cmd = new SqlCommand("BuxSprKdrAdd", con);
        // указать тип команды
       // cmd.CommandType = CommandType.StoredProcedure;
        // создать коллекцию параметров
        ...
        <asp:Panel ID="PanelTop" runat="server" BorderStyle="Double"
            ...
            <asp:TextBox ID="BoxTit"
             ...
             <obout:Grid ID="GridKdr" runat="server"
                ...
                <Columns>
                    <obout:Column ID="Column00" DataField="NKZIDN" HeaderText="ИДН" Width="0%" Visible="false" />
                    <obout:Column ID="Column01" DataField="NKZBEG" HeaderText="ДАТА ВЗЫСК" DataFormatString="{0:dd.MM.yyyy}" ApplyFormatInEditMode="true" Width="10%" />
                    <obout:Column ID="Column02" DataField="NKZMEM" HeaderText="ФАМИЛИЯ И.О." Width="70%" />
                    <obout:Column ID="Column03" DataField="NKZEND" HeaderText="ДАТА СНЯТ" DataFormatString="{0:dd.MM.yyyy}" ApplyFormatInEditMode="true" Width="10%" />
