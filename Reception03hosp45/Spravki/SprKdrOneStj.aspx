...
<script runat="server">
    ...
    void GetGrid()
    {
        ...
        con.Open();
        // создание команды
        SqlCommand cmd = new SqlCommand("SELECT KDRSTJ.* FROM KDR INNER JOIN KDRSTJ ON KDR.KDRKOD=KDRSTJ.KDRKOD WHERE KDR.KDRKOD=" + KdrKod, con);
    ...
    {
        StjBeg = Convert.ToString(e.Record["STJBEG"]);
        StjEnd = Convert.ToString(e.Record["STJEND"]);
        StjNam = Convert.ToString(e.Record["STJNAM"]);
        StjKol = Convert.ToString(e.Record["STJKOL"]);
        StjMem = Convert.ToString(e.Record["STJMEM"]);
        ...
        // создание команды
        SqlCommand cmd = new SqlCommand("INSERT INTO KDRSTJ (KDRKOD,STJBEG,STJEND,STJKOL,STJMEM,STJNAM) " +
                                        "VALUES (@KDRKOD,CONVERT(DATETIME,@STJBEG,103),CONVERT(DATETIME,@STJEND,103),@STJKOL,@STJMEM,@STJNAM)", con);
        ...
        cmd.Parameters.Add("@STJBEG", SqlDbType.VarChar).Value = StjBeg;
        cmd.Parameters.Add("@STJEND", SqlDbType.VarChar).Value = StjEnd;
        cmd.Parameters.Add("@STJNAM", SqlDbType.VarChar).Value = StjNam;
        cmd.Parameters.Add("@STJKOL", SqlDbType.VarChar).Value = StjKol;
        cmd.Parameters.Add("@STJMEM", SqlDbType.VarChar).Value = StjMem;
    ...
    {
        StjIdn = Convert.ToInt32(e.Record["STJIDN"]);
        StjBeg = Convert.ToString(e.Record["STJBEG"]);
        StjEnd = Convert.ToString(e.Record["STJEND"]);
        StjNam = Convert.ToString(e.Record["STJNAM"]);
        StjKol = Convert.ToString(e.Record["STJKOL"]);
        ...
        // создание команды
        SqlCommand cmd = new SqlCommand("UPDATE KDRSTJ SET STJBEG=CONVERT(DATETIME,@STJBEG,103),STJEND=CONVERT(DATETIME,@STJEND,103),STJNAM=@STJNAM,STJKOL=@STJKOL,STJMEM=@STJMEM WHERE STJIDN=@STJIDN", con);
        //   cmd = new SqlCommand("BuxSprKdrAdd", con);
        // указать тип команды
        //      cmd.CommandType = CommandType.StoredProcSemre;
        // создать коллекцию параметров

        cmd.Parameters.Add("@STJIDN", SqlDbType.VarChar).Value = StjIdn;
        cmd.Parameters.Add("@KDRKOD", SqlDbType.VarChar).Value = KdrKod;
        cmd.Parameters.Add("@STJBEG", SqlDbType.VarChar).Value = StjBeg;
        cmd.Parameters.Add("@STJEND", SqlDbType.VarChar).Value = StjEnd;
        cmd.Parameters.Add("@STJNAM", SqlDbType.VarChar).Value = StjNam;
    ...
    void DeleteRecord(object sender, GridRecordEventArgs e)
    {
        ...
        // создание команды
        SqlCommand cmd = new SqlCommand("DELETE FROM KDRSTJ WHERE STJIDN=@STJIDN", con);
        //   cmd = new SqlCommand("BuxSprKdrAdd", con);
        // указать тип команды
       // cmd.CommandType = CommandType.StoredProcedure;
        // создать коллекцию параметров
        ...
        <asp:Panel ID="PanelTop" runat="server" BorderStyle="Double"
            ...
            <asp:TextBox ID="BoxTit"
             ...
             <obout:Grid ID="GridKdr" runat="server"
                ...
                <Columns>
                    <obout:Column ID="Column00" DataField="STJIDN" HeaderText="ИДН" Width="0%" Visible="false" />
                    <obout:Column ID="Column01" DataField="STJNAM" HeaderText="РАБОТА" Width="20%" />
                    <obout:Column ID="Column02" DataField="STJBEG" HeaderText="НАЧАЛО" DataFormatString="{0:dd.MM.yyyy}" ApplyFormatInEditMode="true" Width="10%" />
                    <obout:Column ID="Column03" DataField="STJEND" HeaderText="КОНЕЦ" DataFormatString="{0:dd.MM.yyyy}" ApplyFormatInEditMode="true" Width="10%" />
                    <obout:Column ID="Column04" DataField="STJKOL" HeaderText="СТАЖ" Width="20%" />
                    <obout:Column ID="Column05" DataField="STJMEM" HeaderText="ПРИМЕЧАНИЕ" Width="30%" />
