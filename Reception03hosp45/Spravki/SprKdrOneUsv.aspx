...
<script runat="server">
    ...
    void GetGrid()
    {
        ...
        con.Open();
        // создание команды
        SqlCommand cmd = new SqlCommand("SELECT KDRUSV.* FROM KDR INNER JOIN KDRUSV ON KDR.KDRKOD=KDRUSV.KDRKOD WHERE KDR.KDRKOD=" + KdrKod, con);
    ...
    {
        UsvDat = Convert.ToString(e.Record["USVDAT"]);
        UsvGde = Convert.ToString(e.Record["USVGDE"]);
        UsvNam = Convert.ToString(e.Record["USVNAM"]);
        ...
        // создание команды
        SqlCommand cmd = new SqlCommand("INSERT INTO KDRUSV (KDRKOD,USVDAT,USVGDE,USVNAM) " +
                                        "VALUES (@KDRKOD,CONVERT(DATETIME,@USVDAT,103),@USVGDE,@USVNAM)", con);
        ...
        cmd.Parameters.Add("@USVDAT", SqlDbType.VarChar).Value = UsvDat;
        cmd.Parameters.Add("@USVGDE", SqlDbType.VarChar).Value = UsvGde;
        cmd.Parameters.Add("@USVNAM", SqlDbType.VarChar).Value = UsvNam;
    ...
    {
        UsvIdn = Convert.ToInt32(e.Record["USVIDN"]);
        UsvDat = Convert.ToString(e.Record["USVDAT"]);
        UsvGde = Convert.ToString(e.Record["USVGDE"]);
        UsvNam = Convert.ToString(e.Record["USVNAM"]);
        ...
        // создание команды
        SqlCommand cmd = new SqlCommand("UPDATE KDRUSV SET USVDAT=CONVERT(DATETIME,@USVDAT,103),USVGDE=@USVGDE,USVNAM=@USVNAM WHERE USVIDN=@USVIDN", con);
        //   cmd = new SqlCommand("BuxSprKdrAdd", con);
        // указать тип команды
        //      cmd.CommandType = CommandType.StoredProcSemre;
        // создать коллекцию параметров

        cmd.Parameters.Add("@USVIDN", SqlDbType.VarChar).Value = UsvIdn;
        cmd.Parameters.Add("@USVDAT", SqlDbType.VarChar).Value = UsvDat;
        cmd.Parameters.Add("@USVGDE", SqlDbType.VarChar).Value = UsvGde;
        cmd.Parameters.Add("@USVNAM", SqlDbType.VarChar).Value = UsvNam;
        // ------------------------------------------------------------------------------заполняем второй уровень
    ...
    void DeleteRecord(object sender, GridRecordEventArgs e)
    {
        ...
        // создание команды
        SqlCommand cmd = new SqlCommand("DELETE FROM KDRUSV WHERE USVIDN=@USVIDN", con);
        //   cmd = new SqlCommand("BuxSprKdrAdd", con);
        // указать тип команды
       // cmd.CommandType = CommandType.StoredProcedure;
        // создать коллекцию параметров
        ...
        <asp:Panel ID="PanelTop" runat="server" BorderStyle="Double"
            ...
            <asp:TextBox ID="BoxTit"
             ...
             <obout:Grid ID="GridKdr" runat="server"
                ...
                <Columns>
                    <obout:Column ID="Column00" DataField="USVIDN" HeaderText="ИДН" Width="0%" Visible="false" />
                    <obout:Column ID="Column01" DataField="USVDAT" HeaderText="ДАТА КУРСА" DataFormatString="{0:dd.MM.yyyy}" ApplyFormatInEditMode="true" Width="10%" />
                    <obout:Column ID="Column02" DataField="USVGDE" HeaderText="ГДЕ" Width="30%" />
                    <obout:Column ID="Column03" DataField="USVNAM" HeaderText="НАЗВАНИЕ КУРСА" Width="50%" />
