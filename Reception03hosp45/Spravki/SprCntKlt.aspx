...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <script runat="server">
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                Node rootNode = new Node() { Text = "СТРАХОВЫЕ КОМПАНИИ", Expanded = true };
                OboutTree.Nodes.Add(rootNode);

                foreach (DataRow row in ds.Tables["HspSprCnt"].Rows)
                {
                    Html = Convert.ToString(row["CNTNAM"]);

                    Node newNode = new Node();
                    newNode.Text = Html;
                    newNode.ImageUrl = "/Icon/engineer-icon.png";
                    newNode.Value = Convert.ToString(row["CNTKEY"]);
        ...
        private DataSet GetTree()
        {
            ...
            SqlCommand cmd = new SqlCommand("HspSprCnt", con);
            cmd = new SqlCommand("HspSprCnt", con);
            // указать тип команды
 ...
 //           if (NodLen > 5) NodLen = NodLen + 6;
            ...
            // создание команды
            SqlCommand cmd = new SqlCommand("HspSprCntPop", con);
            cmd = new SqlCommand("HspSprCntPop", con);
            // указать тип команды
            cmd.CommandType = CommandType.StoredProcedure;
            // создать коллекцию параметров
            cmd.Parameters.Add("@BUXFRM", SqlDbType.VarChar).Value = BuxFrm;
            ...
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            // заполняем DataSet из хран.процедуры.
            da.Fill(ds, "HspSprCntPop");

            //=====================================================================================
            foreach (DataRow row in ds.Tables["HspSprCntPop"].Rows)
            {
                Html = Convert.ToString(row["CNTNAM"]);
...
<%-- ============================  JAVA ============================================ --%>
     <script type="text/javascript">
         ...
         function onNodeSelect(sender, args) {
          ...
          //                alert("NodTxt=" + NodTxt);
             if (NodKey.length == 5 || NodKey.length == 11)
                 mySpl.loadPage("RightContent", "SprCntKltGrd.aspx?NodKey=" + NodKey + "&NodTxt=" + NodTxt);
      //           mySpl.loadPage("RightContent", "SprKltGrdDtl.aspx?NodKey=" + NodKey + "&NodTxt=" + NodTxt);
         }
...
<!--  источники -----------------------------------------------------------  -->    
 <asp:Panel ID="PanelLeft" runat="server" ScrollBars="None" Style="border-style: double; left: 10px;
            left: 0px; position: relative; top: 0px; width: 100%; height: 550px;">

        <asp:TextBox ID="TxtUsl" 
             Text="                                                            Справочник фирм и договоров" 
             BackColor="#0099FF"  
             Font-Names="Verdana" 
             Font-Size="20px" 
             Font-Bold="True" 
