...
<%-- ============================  JAVA ============================================ --%>
      <script type="text/javascript">
        ...
        function GridDeb_ClientUpdate(sender, record) {
            ...
            //     ob_post.UpdatePanel("CallbackPanelPay");
            //       ob_post.post(null, "CreateGridKrd", function() { });
            //        ob_post.UpdatePanel("CallbackPanelMat");
 ...
 //           __doPostBack('callPostBack', jsVar);
  ...
  //                   alert(TekMol);
        ...
        function GridDebLoad() {
            var TekAccDeb = BoxAccDeb.value();
            var TekMolDeb = BoxMolDeb.value();
            var TekDocIdn = document.getElementById('MainContent_parDocIdn').value;
    //        var TekAccKrd = BoxAccKrd.value();
    //        var TekMolKrd = BoxMolKrd.value();

           //        alert("TekAcc=" + TekAccDeb);
           //        alert("TekMol=" + TekMolDeb);
           //        alert("TekDocIdn=" + TekDocIdn);

                   mySplDeb.loadPage("BottomContent", "BuxDocPrmSplDeb.aspx?DocIdn=" + TekDocIdn +
                                               "&TekAccDeb=" + TekAccDeb + "&TekMolDeb=" + TekMolDeb + "&TekPrxIdn=0");
            GridKrdLoad();
        }

          function GridKrdLoad() {
    //        alert("GridKrd_ClientUpdate");
    //        var TekAccKrd = BoxAccKrd.value();
    //        var TekMolKrd = BoxMolKrd.value();
            //		        alert("GrfDlg=" + GrfDlg);
              mySplKrd.loadPage("BottomContent", "BuxDocPrmSplKrd.aspx?DocIdn=" + document.getElementById('MainContent_parDocIdn').value);
        }
...
*/
            mySplDeb.loadPage("BottomContent", "BuxDocPrmSplDeb.aspx?DocIdn=" + document.getElementById('MainContent_parDocIdn').value +
                                               "&TekAccDeb=" + TekAccDeb + "&TekMolDeb=" + TekMolDeb + "&TekPrxIdn=0");
        }
  ...
  //          GlvDocPrv = Convert.ToString(Request.QueryString["GlvDocPrv"]);
            ...
            sdsAccDeb.SelectCommand = "SELECT ACCKOD FROM TABACC WHERE ACCPRV=1 AND ACCKOD LIKE '1310%' AND ACCFRM=" + BuxFrm + " ORDER BY ACCKOD";
...
//            sdsMolDeb.SelectCommand = "SELECT MOLKOD,FI AS MOLNAM FROM SprMolKdr WHERE MolFrm=" + BuxFrm + " ORDER BY FI";
            sdsMolDeb.SelectCommand = "SELECT BUXKOD AS MOLKOD,FI AS MOLNAM FROM SprBuxKdr WHERE BUXMOL=1 AND BUXFRM=" + BuxFrm + " ORDER BY FI";
            ...
            sdsMolKrd.SelectCommand = "SELECT BUXKOD AS MOLKOD,FI AS MOLNAM FROM SprBuxKdr WHERE BUXMOL=1 AND BUXFRM=" + BuxFrm + " ORDER BY FI";
            ...
            if (!Page.IsPostBack)
            {
                if (GlvDocIdn == 0)
                {
                    ...
                    cmd.Parameters.Add("@GLVDOCTYP", SqlDbType.VarChar).Value = "Прм";
            ...
            sdsPrxDeb.ConnectionString = WebConfigurationManager.ConnectionStrings[MdbNam].ConnectionString; ;
 //           sdsPrxDeb.SelectCommand = "SELECT TabDoc.DOCIDN AS IDNPRX, STR(TabDoc.DOCNUM, 5) + ' от  ' + CONVERT(VARCHAR(10),TabDoc.DOCDAT, 104) + ' - ' + SprOrg.ORGNAM AS NAMPRX " +
 //                                     "FROM  TabDoc INNER JOIN SprOrg ON TabDoc.DOCDEBSPRVAL=SprOrg.ORGKOD AND TabDoc.DOCFRM=SprOrg.ORGFRM " +
 //                                     "WHERE TabDoc.DOCTYP='Прх' AND TabDoc.DOCFRM ='" + BuxFrm + "' AND TabDoc.DOCDAT > DATEADD(DAY,- 15,Convert.ToDateTime('"+ TxtDocDat.Text +"')) " +
 //                                     "ORDER BY TabDoc.DOCDAT";
            sdsPrxDeb.SelectCommand = "SELECT TabDoc.DOCIDN AS IDNPRX, STR(TabDoc.DOCNUM, 5) + ' от  ' + CONVERT(VARCHAR(10),TabDoc.DOCDAT, 104) + ' - ' + SprOrg.ORGNAM AS NAMPRX " +
                                      "FROM  TabDoc INNER JOIN SprOrg ON TabDoc.DOCKRDSPRVAL=SprOrg.ORGKOD AND TabDoc.DOCFRM=SprOrg.ORGFRM " +
                                      "WHERE (TabDoc.DOCTYP='Ввд' OR TabDoc.DOCTYP='Прх') AND TabDoc.DOCFRM ='" + BuxFrm + "' AND TabDoc.DOCDAT > DATEADD(DAY,- 30,GETDATE()) " +
                                      "ORDER BY TabDoc.DOCDAT";
        }
                        ...
                        <TopPanel HeightMin="40" HeightMax="100" HeightDefault="60">
                            <Content>
                                ...
                                <asp:UpdatePanel runat="server" ID="CascadingPanelDeb">
                                    <ContentTemplate>
                                        <asp:Label id="Lab001" Text="СЧЕТ:" runat="server" Width="5%"/> 
                                        <obout:ComboBox runat="server" ID="BoxAccDeb" Width="12%" Height="400"
                                       ...
                                       <asp:Label id="Lab002" Text="МОЛ:" runat="server" Width="5%"/> 
                                        <obout:ComboBox runat="server" ID="BoxMolDeb" Width="25%" Height="300"
                                        ...
                                        <asp:Label id="Lab003" Text="№.ПРХ:" runat="server" Width="7%"/> 
                                        <obout:ComboBox runat="server" ID="BoxPrxDeb" Width="40%" Height="300"
                                            FolderStyle="/Styles/Combobox/Plain"
                                            AutoPostBack="false"
                                            DataSourceID="sdsPrxDeb" DataTextField="NAMPRX" DataValueField="IDNPRX">
                                            <ClientSideEvents OnSelectedIndexChanged="BoxPrxDeb_SelectedIndexChanged" />
                                       </obout:ComboBox>
                            ...
                            <Content>
                               ...
                               <asp:UpdatePanel runat="server" ID="UpdatePanel1">
                                    <ContentTemplate>
                                        ...
                                        <obout:ComboBox runat="server" ID="BoxMolKrd" Width="45%" Height="300"
                                            FolderStyle="/Styles/Combobox/Plain"
                                            AutoPostBack="false"
                                            DataSourceID="sdsMolKrd" DataTextField="MOLNAM" DataValueField="MOLKOD">
                                       </obout:ComboBox>
