...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        // -------изменение как EXCEL-------------------------------------------------------------------          
        function markAsFocused001(textbox) {
       //                alert("markAsFocused=");
            textbox.className = 'excel-textbox-focused';
        }
        function markAsFocused002(textbox) {
            //                alert("markAsFocused=");
            textbox.className = 'excel-textbox-focused';
        }
        function markAsFocused003(textbox) {
            //                alert("markAsFocused=");
            textbox.className = 'excel-textbox-focused';
        }
        /*------------------------- при нажатии на поле текст --------------------------------*/
        function markAsBlured001(textbox, dataField, rowIndex) {
        //               alert("markAsBlured=");
            ...
            DatDocIdn = GridLabOne.Rows[rowIndex].Cells['LABUSLIDN001'].Value;
            DatDocVal = textbox.value;
            onChangeUpd(DatDocRek, DatDocVal, DatDocTyp, DatDocIdn);
        }
        function markAsBlured002(textbox, dataField, rowIndex) {
            //               alert("markAsBlured=");
            //           alert("textbox=" + textbox.value + " dataField=" + dataField + " rowIndex=" + rowIndex);
            var DatDocIdn;
            var DatDocRek = 'USLDTLVAL';
            ...
            //          DatDocRek = GridLabOne.Rows[rowIndex].Cells['LabUSLREK'].Value;
            DatDocIdn = GridLabOne.Rows[rowIndex].Cells['LABUSLIDN002'].Value;
            DatDocVal = textbox.value;
            onChangeUpd(DatDocRek, DatDocVal, DatDocTyp, DatDocIdn);
        }
        function markAsBlured003(textbox, dataField, rowIndex) {
            //               alert("markAsBlured=");
            //           alert("textbox=" + textbox.value + " dataField=" + dataField + " rowIndex=" + rowIndex);
            var DatDocIdn;
            var DatDocRek = 'USLDTLVAL';
            ...
            //          DatDocRek = GridLabOne.Rows[rowIndex].Cells['LabUSLREK'].Value;
            DatDocIdn = GridLabOne.Rows[rowIndex].Cells['LABUSLIDN003'].Value;
            DatDocVal = textbox.value;
            onChangeUpd(DatDocRek, DatDocVal, DatDocTyp, DatDocIdn);
        }
...
//            BoxTit.Text = Convert.ToString(ds.Tables[0].Rows[0]["SPRLABNAM"]);
            ...
            {
                GridLabOne.Columns[6].HeaderText = "ВЛАГАЛИЩЕ"; // Convert.ToString(ds.Tables[0].Rows[0]["SPRLABLEN001"]);   // группа
                GridLabOne.Columns[7].HeaderText = "ЦЕРВ.КАНАЛ"; // Convert.ToString(ds.Tables[0].Rows[0]["SPRLABLEN001"]);   // группа
                GridLabOne.Columns[8].HeaderText = "УРЕТРА"; // Convert.ToString(ds.Tables[0].Rows[0]["SPRLABLEN001"]);   // группа
            }
 ...
 //       if (BoxTit.Text == "Запись не найден") return;
        ...
        con.Open();
        // создание команды
        SqlCommand cmd = new SqlCommand("HspAmbLabOneDtl003", con);
        cmd = new SqlCommand("HspAmbLabOneDtl003", con);
        // указать тип команды
        cmd.CommandType = CommandType.StoredProcedure;
       ...
       <%-- ============================  для передач значении  ============================================ --%>
        ...
        <asp:Panel ID="PanelTop" runat="server" BorderStyle="Double"
            ...
            <obout:Grid ID="GridLabOne" runat="server"
                ...
                <Columns>
                    <obout:Column ID="Column10" DataField="LABUSLIDN001" HeaderText="Идн1" ReadOnly="true" Visible="false" Width="0%" />
                    <obout:Column ID="Column11" DataField="LABUSLIDN002" HeaderText="Идн2" ReadOnly="true" Visible="false" Width="0%" />
                    <obout:Column ID="Column12" DataField="LABUSLIDN003" HeaderText="Идн3" ReadOnly="true" Visible="false" Width="0%" />
                    <obout:Column ID="Column13" DataField="LABUSLSAP" HeaderText="Компоненты,элементы" ReadOnly="true" Width="30%" />
                    <obout:Column ID="Column14" DataField="LABUSLEDN" HeaderText="Ед.изм. СИ" ReadOnly="true" Width="10%" />
                    <obout:Column ID="Column15" DataField="LABUSLNORMEN" HeaderText="Пределы физ. исследований" ReadOnly="true" Width="15%" />
                   
                     <obout:Column ID="Column16" DataField="LABUSLVAL001" HeaderText="Результаты" Width="15%">
                        <TemplateSettings TemplateId="TxtEdit001" />
                    </obout:Column>
                    
                    <obout:Column ID="Column17" DataField="LABUSLVAL002" HeaderText="Результаты" Width="15%">
                        <TemplateSettings TemplateId="TxtEdit002" />
                    </obout:Column>
                    
                    <obout:Column ID="Column18" DataField="LABUSLVAL003" HeaderText="Результаты" Width="15%">
                        <TemplateSettings TemplateId="TxtEdit003" />
                    </obout:Column>           
                ...
                <Templates>
                    ...
                    <obout:GridTemplate runat="server" ID="TxtEdit001">
                        <Template>
                            <input type="text" style="font-size: 14px" class="excel-textbox" value="<%# Container.Value %>" ondblclick="markAsDblClick(this, '<%# GridLabOne.Columns[Container.ColumnIndex].DataField %>', <%# Container.PageRecordIndex %>)"
                                onfocus="markAsFocused001(this)" onblur="markAsBlured001(this, '<%# GridLabOne.Columns[Container.ColumnIndex].DataField %>', <%# Container.PageRecordIndex %>)" />
                        </Template>
                    </obout:GridTemplate>

                    <obout:GridTemplate runat="server" ID="TxtEdit002">
                        <Template>
                            <input type="text" style="font-size: 14px" class="excel-textbox" value="<%# Container.Value %>" ondblclick="markAsDblClick(this, '<%# GridLabOne.Columns[Container.ColumnIndex].DataField %>', <%# Container.PageRecordIndex %>)"
                                onfocus="markAsFocused002(this)" onblur="markAsBlured002(this, '<%# GridLabOne.Columns[Container.ColumnIndex].DataField %>', <%# Container.PageRecordIndex %>)" />
                        </Template>
                    </obout:GridTemplate>

                    <obout:GridTemplate runat="server" ID="TxtEdit003">
                        <Template>
                            <input type="text" style="font-size: 14px" class="excel-textbox" value="<%# Container.Value %>" ondblclick="markAsDblClick(this, '<%# GridLabOne.Columns[Container.ColumnIndex].DataField %>', <%# Container.PageRecordIndex %>)"
                                onfocus="markAsFocused003(this)" onblur="markAsBlured003(this, '<%# GridLabOne.Columns[Container.ColumnIndex].DataField %>', <%# Container.PageRecordIndex %>)" />
                        </Template>
                    </obout:GridTemplate>
                </Templates>
        ...
        <asp:Panel ID="PanelBot" runat="server" BorderStyle="Double" Style="left: 0px%; position: relative; top: -10px; width: 100%; height: 30px;">
            <table border="0" cellspacing="0" width="100%" cellpadding="0">
                <tr>
                    <td style="width: 50%">
                        <asp:Label ID="Label7" Text="ОТВЕТСТВЕН:" runat="server" Width="25%" Font-Bold="true" />
                        <obout:ComboBox runat="server"
                            ID="BoxKto"
                            Width="50%"
                            Height="200"
