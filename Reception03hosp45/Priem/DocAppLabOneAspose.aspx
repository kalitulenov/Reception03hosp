...
<script runat="server">
    ...
    */
    ...
    void GetGrid()
    {
        ...
        con.Open();
        // создание команды
        SqlCommand cmd = new SqlCommand("HspAmbLabOneDtlSpire", con);
        cmd = new SqlCommand("HspAmbLabOneDtlSpire", con);
        // указать тип команды
        ...
        da.Fill(ds, "HspAmbLabOneDtlSpire");

        if (ds.Tables[0].Rows.Count > 0)
        {
            LabXlsNam = Convert.ToString(ds.Tables[0].Rows[0]["XLSNAMSBL"]);
            LabXlsImg = Convert.ToString(ds.Tables[0].Rows[0]["LABNAMXLS"]);
            LabXlsOrg = Convert.ToString(ds.Tables[0].Rows[0]["ORGKLTNAM"]);
            LabXlsAdr = Convert.ToString(ds.Tables[0].Rows[0]["ORGKLTADR"]);
            LabXlsDat = Convert.ToString(ds.Tables[0].Rows[0]["GRFDAT"]);
            LabXlsFio = Convert.ToString(ds.Tables[0].Rows[0]["GRFPTH"]);
            LabXlsIin = Convert.ToString(ds.Tables[0].Rows[0]["GRFIIN"]);
            LabXlsBrt = Convert.ToString(ds.Tables[0].Rows[0]["GRFBRT"]);
            LabXlsDoc = Convert.ToString(ds.Tables[0].Rows[0]["GRFDOC"]);

            //      LabXlsOrgXyz = Convert.ToString(ds.Tables[0].Rows[0]["XLSORGNAM"]);
            //      LabXlsAdrXyz = Convert.ToString(ds.Tables[0].Rows[0]["XLSORGADR"]);
            //      LabXlsDatXyz = Convert.ToString(ds.Tables[0].Rows[0]["XLSDAT"]);
            //      LabXlsFioXyz = Convert.ToString(ds.Tables[0].Rows[0]["XLSFIO"]);
            //      LabXlsBrtXyz = Convert.ToString(ds.Tables[0].Rows[0]["XLSBRT"]);
            LabXlsDocXyz = Convert.ToString(ds.Tables[0].Rows[0]["XLSDOC"]);
            parXlsFio.Value = LabXlsFio;
        }
        ds.Dispose();
        //            con.Close();
        ...
        if (LabXlsImg == "")
        {
            ...
            // СФОРМИРОВАТЬ ПУТЬ ===========================================================================================
            string Papka = @"C:\BASELAB\" + Convert.ToInt32(BuxFrm).ToString("D5") + @"\" + Convert.ToDateTime(DateTime.Today).ToString("yyyy.MM");
            XlsFil = Papka + @"\" + LabXlsIin + "_" + Convert.ToInt32(AmbUslIdn).ToString("D10");
            parXlsFil.Value = XlsFil + ".xls";
            parXlsFilPdf.Value = XlsFil + ".pdf";
            LabXlsImg = XlsFil;

            //  показывает в виде HTML без корректировки             
            //              string XlsFil = Papka + @"\" + LabXlsFio.Substring(0, 12) + "_" + Convert.ToInt32(AmbUslIdn).ToString("D10") + ".html";
            ...
            string Sablon = @"C:\BASELAB\LabSablon\" + LabXlsNam + ".xls";
        ...
        else
        {
            ...
            parXlsFil.Value = LabXlsImg + ".xls";
            parXlsFilPdf.Value = LabXlsImg + ".pdf";
            GridWeb1.ImportExcelFile(parXlsFil.Value);
        }
        ...
        sheet.Cells["B6"].PutValue(LabXlsDat.Substring(0, 10));
        sheet.Cells["B2"].PutValue(LabXlsOrg);
        sheet.Cells["B7"].PutValue(LabXlsFio);
        sheet.Cells["B8"].PutValue(LabXlsBrt.Substring(0, 10));

        if (LabXlsBrt.Length > 9) sheet.Cells["B8"].PutValue(LabXlsBrt.Substring(0, 10));
        ...
        for (i = 2; i < 50; i++)
        {
            ...
            if (Pol.IndexOf("Врач") > -1)
            {
                sheet.Cells[i, 1].PutValue(LabXlsDoc);
                //      break;
            }
            if (Pol.IndexOf("ТАӘ (ФИО)") > -1)
            {
                sheet.Cells[i, 1].PutValue(LabXlsFio);
                //      break;
            }
            if (Pol.IndexOf("рождения") > -1)
            {
                sheet.Cells[i, 1].PutValue(LabXlsBrt.Substring(0, 10));
                //      break;
            }
            if (Pol.IndexOf("Найменование организации") > -1)
            {
                sheet.Cells[i, 0].PutValue(LabXlsOrg);
                //      break;
            }
        ...
        for (i = 10; i < cells.MaxDataRow; i++)
        {
            ...
            //   if (cellDoc.StringValue == "Врач") sheet.Cells[i,1].PutValue(LabXlsDoc);
            ...
            for (int j = 0; j <= cells.MaxDataColumn; j++)
            {
                ...
                */
                ...
                //cells.SetRowHeight(i, 17 * (L * 1.1 / ColWdt + 1));  //пример
