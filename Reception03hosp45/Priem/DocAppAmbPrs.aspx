...
<script runat="server">
 ...
 //       else PrsUsl = Convert.ToInt32(e.Record["PRSUSL"]);
        ...
        if (e.Record["PRSOBSTXT"] == null | e.Record["PRSOBSTXT"] == "") PrsMem = "";
        else PrsMem = Convert.ToString(e.Record["PRSOBSTXT"]);
        ...
        cmd.Parameters.Add("@PRSUSL", SqlDbType.Int, 4).Value = 0;
...
//                                        e.Text + "' " + "ORDER BY SprUsl.UslNam";
 ...
 //       string commandText = "SELECT * FROM SPRUSL ";
        string commandText = "SELECT UslIdn,UslTrf,UslKod,UslNam," +
                       "CASE WHEN ISNULL(UslFrmZen, 0)>0 THEN 'Здесь' ELSE CASE WHEN LEN(ISNULL(UslFrmIin,'')) = 0 THEN '' ELSE SprOrgHsp.ORGHSPNAMSHR END END AS GDE " +
                       "FROM SprUsl INNER JOIN SprUslFrm ON SprUsl.UslKod=SprUslFrm.UslFrmKod " +
                                   ...
                                   "LEFT OUTER JOIN SprOrgHsp ON SprUslFrm.UslFrmIin=SprOrgHsp.ORGHSPBIN " +
...
<body>
    ...
    <form id="form1" runat="server">
        ...
        <asp:Panel ID="PanelMid" runat="server" BorderStyle="Double" ScrollBars="Vertical"
            Style="left: 0%; position: relative; top: 0px; width: 100%; height: 390px;">


            <%-- ============================  шапка экрана ============================================ 
                
                                 <obout:Column ID="Column4" DataField="PRSUSL" HeaderText="НАПРАВЛЕНИЯ" Width="36%" Wrap="true" >
            ...
            <obout:Grid ID="GridNap" runat="server"
                ...
                <Columns>
                    ...
                    <obout:Column ID="Column3" DataField="PRSTRF" HeaderText="ТАРИФ" Width="7%" >
                           <TemplateSettings EditTemplateId="TemplateEditTrfNam" />
                    </obout:Column>

                    <obout:Column ID="Column5" DataField="PRSOBSTXT" HeaderText="ПРИМЕЧАНИЕ" Width="63%"  Align="left" Wrap="true" />
                    <obout:Column ID="Column6" DataField="ORGNAMSHR" HeaderText="ГДЕ" Width="10%"  Align="center" ReadOnly="true"  ItemStyle-Font-Bold="true"/>
                    <obout:Column ID="Column7" HeaderText="Коррек  Удаление" Width="7%" AllowEdit="true" AllowDelete="true" runat="server">
				           <TemplateSettings TemplateId="editBtnTemplate" EditTemplateId="updateBtnTemplate" />
                    </obout:Column>	  

                    <obout:Column ID="Column8" DataField="PRTFLG" HeaderText="ПЕЧ" Width="4%" ReadOnly="true" >
				         <TemplateSettings TemplateId="TemplatePrt" />
				    </obout:Column>	
                    
                    <obout:Column ID="Column9" DataField="SBLFLG" HeaderText="ШАБЛОН" Width="4%" ReadOnly="true" >
                   ...
                   <obout:GridTemplate runat="server" ID="TemplateSbl">
                       <Template>
                          <input type="button" id="btnSbl" class="tdTextSmall" value="Шаблон" onmousedown="GridPrs_sbl(<%# Container.PageRecordIndex %>)"/>
 					   </Template>
                    </obout:GridTemplate>

                   <obout:GridTemplate runat="server" ID="TemplatePrt">
                       <Template>
                          <input type="button" id="btnSbl" class="tdTextSmall" value="Печ" onmousedown="GridPrs_prt(<%# Container.PageRecordIndex %>)"/>
 					   </Template>
                    </obout:GridTemplate>
