...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function markAsBlured(textbox, dataField, rowIndex) {
            var DatDocIdn;
            var DatDocRek = 'LOCMEM';
            var DatDocVal;
            var DatDocTyp = 'Txt';

            textbox.className = 'excel-textbox';

            GridLOC.Rows[rowIndex].Cells[dataField].Value = textbox.value;

            DatDocIdn = GridLoc.Rows[rowIndex].Cells['LOCIDN'].Value;
        ...
        function saveCheckBoxChanges(element, rowIndex) {
            parDocIdn.value = GridLoc.Rows[rowIndex].Cells['LOCIDN'].Value;
        ...
        function onChangeUpd(DatDocRek, DatDocVal, DatDocTyp, DatDocIdn) {
            ...
            var DatDocTab = 'AMBLOC';
            var DatDocKey = 'LOCIDN';
...
//        AmbCrdIdn = (string)Session["AmbCrdIdn"];
    ...
    void getGrid()
    {
        ...
        con.Open();
        SqlCommand cmd = new SqlCommand("HspAmbLocIdn", con);
        // указать тип команды
     ...
     //       if (String.IsNullOrEmpty(Convert.ToString(ds.Tables[0].Rows[0]["LOCLEN003"]))) LenCol = "0";
    //        else 
            LenCol = Convert.ToString(ds.Tables[0].Rows[0]["LOCLEN003"]);

            GridLoc.Columns[1].Width = Convert.ToString(ds.Tables[0].Rows[0]["LOCLEN001"]);
            GridLoc.Columns[2].Width = Convert.ToString(ds.Tables[0].Rows[0]["LOCLEN002"]);
            GridLoc.Columns[3].Width = Convert.ToString(ds.Tables[0].Rows[0]["LOCLEN003"]);
            GridLoc.Columns[4].Width = Convert.ToString(ds.Tables[0].Rows[0]["LOCLEN004"]);
            GridLoc.Columns[5].Width = Convert.ToString(ds.Tables[0].Rows[0]["LOCLEN005"]);
            GridLoc.Columns[6].Width = Convert.ToString(ds.Tables[0].Rows[0]["LOCLEN006"]);

            GridLoc.DataSource = ds;
            GridLoc.DataBind();
        }
...
<body>
    ...
    <form id="form1" runat="server">
        ...
        <asp:HiddenField runat="server" ID="GridLocExcelDeletedIds" />
        <asp:HiddenField runat="server" ID="GridLocExcelData" />
        <%-- ============================  шапка экрана ============================================ --%>
        ...
        <asp:Panel ID="PanelMid" runat="server" BorderStyle="Double" ScrollBars="Vertical"
            ...
            <%-- ============================  шапка экрана ============================================ --%>
            <obout:Grid ID="GridLoc" runat="server"
                CallbackMode="true"
                Serialize="true"
                ...
                <Columns>
                    <obout:Column ID="Column0" DataField="LOCIDN" HeaderText="Идн" Visible="false" Width="0%" />
                    <obout:Column ID="Column1" DataField="LOCNNN" HeaderText="№" Width="10%" />
                    <obout:Column ID="Column2" DataField="LOCTXT001" HeaderText="ПАРАМЕТРЫ" Width="30%" />
                    <obout:Column ID="Column6" DataField="LOCMEM001" HeaderText="ЗНАЧЕНИЯ" Width="60%" >
                ...
                <Templates>
                    <obout:GridTemplate runat="server" ID="TxtEditTemplate">
                        <Template>
                            <input type="text" class="excel-textbox" value="<%# Container.Value %>" ondblclick="markAsDblClick(this, '<%# GridLoc.Columns[Container.ColumnIndex].DataField %>', <%# Container.PageRecordIndex %>)"
                                onfocus="markAsFocused(this)" onblur="markAsBlured(this, '<%# GridLoc.Columns[Container.ColumnIndex].DataField %>', <%# Container.PageRecordIndex %>)" />
                        </Template>
                    </obout:GridTemplate>
                </Templates>
        ...
        <script type="text/javascript">
            window.onload = function () {
                GridLoc.convertToExcel(
                    ['ReadOnly', 'TextBox', 'TextBox', 'MultiLineTextBox', 'ComboBox', 'TextBox', 'CheckBox', 'Actions'],
                    '<%=GridLocExcelData.ClientID %>',
                    '<%=GridLocExcelDeletedIds.ClientID %>'
                );
         }
        </script>
    </form>
